...
type DiagramProps = {
  ...
  steps: string[];
  imageTitle?: string;
  children: (imageLabel: string) => ReactNode;
};
...
function DiagramFrame({
  ...
  return (
    <figure className="adaptive-diagram">
      ...
      {children(`${imageTitle}. ${description}`)}
...
export function RetryTimelineDiagram() {
  ...
  return <DiagramFrame title={title} description={description} steps={["The initial request is sent at zero seconds and receives 429.", "Retry one waits about half a second plus jitter.", "Retry two waits about one second plus jitter.", "Retry three waits about two seconds plus jitter.", "The client stops when its bounded attempt budget is exhausted."]}>{(imageLabel) => <svg viewBox="0 0 760 300" role="img" aria-label={imageLabel}><line className="timeline-axis" x1="62" y1="188" x2="706" y2="188" />{attempts.map((item) => <g key={item.label} className="timeline-attempt"><line x1={item.x} y1="188" x2={item.x} y2={188 - item.height} /><circle cx={item.x} cy={188 - item.height} r="13" /><text x={item.x} y="222" textAnchor="middle">{item.label}</text><text x={item.x} y="248" textAnchor="middle">{item.delay}</text></g>)}</svg>}</DiagramFrame>;
}

export function RateLimitWindowDiagram() {
  const title = "Requests inside a fixed allowance";
  const description = "During a rate-limit window, requests consume a fixed allowance. Once the allowance is exhausted, later calls receive 429 until the window resets.";
  return <DiagramFrame title={title} description={description} imageTitle="Requests inside one rate-limit window" steps={["A request window begins with five available request slots.", "Each accepted request uses one slot.", "The sixth request is over the example limit and receives 429.", "The client waits instead of sending a tight retry loop.", "A new request becomes eligible after the window resets."]}>{(imageLabel) => <svg viewBox="0 0 760 300" role="img" aria-label={imageLabel}><rect className="window-band" x="42" y="54" width="470" height="184" rx="28" /><text className="window-title" x="72" y="90">Current window · limit 5</text>{[0, 1, 2, 3, 4].map((index) => <g key={index} className="request-token"><circle cx={104 + index * 86} cy="154" r="30" /><text x={104 + index * 86} y="161" textAnchor="middle">{index + 1}</text></g>)}<path className="window-divider" d="M542 44 V248" /><g className="blocked-request"><circle cx="624" cy="128" r="36" /><text x="624" y="135" textAnchor="middle">6</text><text x="624" y="190" textAnchor="middle">429 · wait</text></g></svg>}</DiagramFrame>;
}
